...
import { Upload, FileText, X, CheckCircle2, AlertCircle, ArrowRight, ArrowLeft, Cloud, CreditCard } from 'lucide-react';
...
export const DocumentUploadStep: React.FC<DocumentUploadStepProps> = ({
  ...
  const uploadProgress = documents.length > 0 ? (uploadedFiles.length / documents.length) * 100 : 0;
  const isComplete = uploadedFiles.length >= documents.length;
  const [showPreVerification, setShowPreVerification] = React.useState(false);
  ...
  const handlePreVerificationPayment = () => {
    setShowPreVerification(true);
    ...
    <Card className="shadow-premium bg-gradient-card border-0">
      <CardHeader>
        <CardTitle className="text-2xl flex items-center gap-3">
          ...
          Upload & Pre-Verification
        </CardTitle>
        <CardDescription className="text-base">
          Upload your documents for secure pre-verification. Maximum file size: 5MB per document.
        </CardDescription>
      ...
      <CardContent className="space-y-6">
        ...
        {/* Pre-Verification Fee Notice */}
        {uploadedFiles.length > 0 && !showPreVerification && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-start gap-3">
              <CreditCard className="w-5 h-5 text-blue-600 mt-0.5" />
              <div className="flex-1">
                <h4 className="font-semibold text-blue-800 mb-1">Pre-Verification Available</h4>
                <p className="text-sm text-blue-700 mb-3">
                  Get your documents verified in advance for faster processing. 
                  Pre-verification fee: <strong>BD 10.000</strong>
                </p>
                <Button
                  onClick={handlePreVerificationPayment}
                  size="sm"
                  className="bg-blue-600 hover:bg-blue-700"
                >
                  Pay for Pre-Verification
                </Button>
              </div>
            </div>
          </div>
              ...
              {uploadedFiles.map((file, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant={showPreVerification ? "success" : "secondary"}>
                      <CheckCircle2 className="w-3 h-3 mr-1" />
                      {showPreVerification ? 'Pre-Verified' : 'Uploaded'}
                    </Badge>
                    <Button
                      variant="ghost"
                      size="sm"
